...
if (isset($_POST['generar_reporte']))
{
	...
	fprintf($salida, chr(0xEF).chr(0xBB).chr(0xBF));
	  ...
	  $sql = "SELECT CAST(Movil1 AS VARCHAR(11))  
	  + cast(',' as varchar(1)) + CAST('0' AS VARCHAR(1)) 
	  + cast(',' as varchar(1)) 
	  + CAST(Case When canal= 'AA'
				  ...
				  Then '''Tu opinion es importante. Ayudanos a valorar la gestion realizada en el canal Mi Movistar ingresando en http://bit.ly/2XKtnMV'''
	   			...
	   			FROM Broadcast_call where Fecha >= '$fecha1' AND Fecha < '$fecha2' + 'INTERVAL 1 DAY' AND canal in ('AA', 'ASI', 'CDS', 'CC', 'MM') 
	   			ORDER BY canal asc ";
